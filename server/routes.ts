...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/client/stage-invoices/:stageId/download-resimple", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      doc.fontSize(9).fillColor('#475569').font('Helvetica');
      doc.text('Subtotal Gs:', labelX, yPos + 26);
      doc.fontSize(9).fillColor('#1e293b').font('Helvetica-Bold');
      ...
      doc.fontSize(8).fillColor('#475569').font('Helvetica');
      doc.text('T.C. (1 USD = Gs):', labelX, yPos + 58);
      doc.fontSize(8).fillColor('#1e293b').font('Helvetica-Bold');
      doc.text(`${exchangeRate.toLocaleString('es-PY')}`, valueColumnX, yPos + 58, { width: valueWidth, align: 'right' });

      // Total destacado con fondo
      doc.rect(totalsX, yPos + 72, totalsBoxWidth, 43).fillColor('#1e3a8a').fill();
      doc.fontSize(11).fillColor('#ffffff').font('Helvetica-Bold');
      doc.text('TOTAL USD:', labelX, yPos + 78);
      doc.text(`USD ${amountUSD.toFixed(2)}`, valueColumnX, yPos + 78, { width: valueWidth, align: 'right' });
      doc.text('TOTAL Gs:', labelX, yPos + 96);
      doc.text(`Gs ${amountPYG.toLocaleString('es-PY')}`, valueColumnX, yPos + 96, { width: valueWidth, align: 'right' });

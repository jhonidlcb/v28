...
export default function Footer() {
  const { data: companyInfo } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!token) return null; // No intentar la petición sin token
      
      const response = await apiRequest("GET", "/api/admin/company-billing-info");
      if (!response.ok) return null;
      return await response.json();
    },
    enabled: false, // Deshabilitar esta query ya que es pública
            ...
            <div className="flex flex-col items-start w-full">
              ...
              <div className="flex items-start gap-4 w-full">
                ...
                <div className="text-sm text-muted-foreground flex-1">
                  ...
                  <p>RUC: {companyInfo?.ruc || "4220058-0"}</p>
                  <p>Timbrado N°: {companyInfo?.timbradoNumber || "556454566"}</p>
                </div>
              </div>
            </div>
